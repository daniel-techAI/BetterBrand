(() => {
  ...
  const hydrateProductPage = () => {
    ...
    if (gallery) {
      gallery.innerHTML = `
        <div class="product-gallery" data-product-gallery-track role="region" aria-label="Product image carousel" tabindex="0">${product.images.map((image, index) => `<div class="product-media ${index === 0 ? "is-featured is-active" : ""}" data-product-slide data-media-id="${index + 1}"><img src="${assetUrl(image)}" alt="${product.title} ${index === 0 ? "product view" : `detail ${index + 1}`}"></div>`).join("")}</div>
